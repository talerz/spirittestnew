...
void AShooterPlayerController::BeginPlay()
{
	...
	if (IsLocalPlayerController())
	{
		...
		if (BulletCounterUI)
		{
			...
			UE_LOG(LogSpiritTest, Error, TEXT("Could not spawn bullet counter widget."));
...
void AShooterPlayerController::OnPossess(APawn* InPawn)
{
	...
	if (APlayerCharacter* ShooterCharacter = Cast<APlayerCharacter>(InPawn))
	{
		// add the player tag
...
void AShooterPlayerController::OnPawnDestroyed(AActor* DestroyedActor)
{
	...
	if (ActorList.Num() > 0)
	{
		...
		if (APlayerCharacter* RespawnedCharacter = GetWorld()->SpawnActor<APlayerCharacter>(CharacterClass, SpawnTransform))
		{
